...
class Atendimento(models.Model):
    ...
    id_escritorio = models.PositiveIntegerField()  # ID sequencial do atendimento dentro do escritório

    def save(self, *args, **kwargs):
        """Adiciona a lógica para o ID único por escritório."""
        if not self.pk:  # Apenas para novos objetos
            ultimo_atendimento = Atendimento.objects.filter(escritorio=self.escritorio).order_by('id_escritorio').last()
            if ultimo_atendimento:
                self.id_escritorio = ultimo_atendimento.id_escritorio + 1
            else:
                self.id_escritorio = 1  # Primeiro atendimento do escritório

        super().save(*args, **kwargs)  # Salva o atendimento no banco de dados

    def __str__(self):
        return f"Atendimento {self.id_escritorio} - {self.nome_empresa}"
